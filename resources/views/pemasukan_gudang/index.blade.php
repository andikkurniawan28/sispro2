...
@section('title')
    {{ ucReplaceUnderscoreToSpace('pemasukan_gudang') }}
@endsection
...
@section('content')
                    ...
                    <div class="card">
                        <div class="card-header">
                            <div class="card-title text-white text-right">
                                <a href="{{ route('pemasukan_gudang.create') }}" class="btn btn-sm btn-primary text-white">Tambah</a>
                            </div>
                        </div>
                        <div class="card-body">
                            <div class="table-responsive">
                                <table class="table table-bordered table-hover mt-4" id="pemasukan_gudang-table" width="100%">
                                    <thead>
                                        <tr>
    ...
    <script type="text/javascript">
        $(function () {
            $('#pemasukan_gudang-table').DataTable({
                order: [
                    [1, 'desc']
                ],
                processing: true,
                serverSide: true,
                ajax: "{{ route('pemasukan_gudang.index') }}",
                columns: [
                    { data: 'kode', name: 'kode' },
        ...
        document.addEventListener("DOMContentLoaded", function() {
            document.addEventListener('click', function(event) {
                if (event.target.classList.contains('delete-btn')) {
                    ...
                    const pemasukan_gudang_id = button.getAttribute('data-id');
                    const pemasukan_gudang_nama = button.getAttribute('data-nama');
                    ...
                    Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            ...
                            form.setAttribute('action', `{{ url('pemasukan_gudang') }}/${pemasukan_gudang_id}`);
