...
@section('title')
    {{ ucReplaceUnderscoreToSpace('akun') }}
@endsection
                    ...
                    <div class="card">
                        <div class="card-header">
                            <div class="card-title text-white text-right">
                                <a href="{{ route('akun.create') }}" class="btn btn-sm btn-primary text-white">Tambah</a>
                            </div>
                        </div>
                        <div class="card-body">
                            <div class="table-responsive">
                                <table class="table table-bordered table-hover mt-4" id="akun-table" width="100%">
                                    <thead>
                                        <tr>
                                            <th>{{ ucReplaceUnderscoreToSpace('kode') }}</th>
                                            <th>{{ ucReplaceUnderscoreToSpace('akun_sub') }}</th>
    ...
    <script type="text/javascript">
        $(function () {
            $('#akun-table').DataTable({
                ...
                columns: [
                    { data: 'kode', name: 'kode' },
                    { data: 'akun_sub_nama', name: 'akun_sub.nama' },
                    { data: 'nama', name: 'nama' },
        ...
        document.addEventListener("DOMContentLoaded", function() {
            document.addEventListener('click', function(event) {
                if (event.target.classList.contains('delete-btn')) {
                    ...
                    const akun_id = button.getAttribute('data-id');
                    const akun_nama = button.getAttribute('data-nama');
                    ...
                    Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            ...
                            form.setAttribute('action', `{{ url('akun') }}/${akun_id}`);
