...
@section('title')
    Detail {{ ucReplaceUnderscoreToSpace('mutasi_antar_gudang') }}
@endsection
                    ...
                    <div class="card" id="invoice-section">
                        <div class="card-body">
                            <div class="invoice-header">
                                <h4 class="invoice-title">Faktur {{ ucReplaceUnderscoreToSpace('mutasi_antar_gudang') }}</h4>
                                <div class="invoice-info">
                                    <div>
                                        <strong>Kode:</strong> {{ $mutasi_antar_gudang->kode }}
                                    </div>
                                    <div>
                                        <strong>Dibuat pada:</strong>
                                        {{ date('d-m-Y H:i:s', strtotime($mutasi_antar_gudang->created_at)) }}
                                    </div>
                                    <div>
                                        <strong>Gudang Asal:</strong>
                                        {{ $mutasi_antar_gudang->gudang_asal->nama }}
                                    </div>
                                    <div>
                                        <strong>Gudang Tujuan:</strong>
                                        {{ $mutasi_antar_gudang->gudang_tujuan->nama }}
                                    </div>
                                    <div>
                                        <strong>Dicatat Oleh:</strong> {{ $mutasi_antar_gudang->user->nama }}
                                        ({{ $mutasi_antar_gudang->user->jabatan->nama }})
                                    </div>
                                </div>
                            </div>
                            <hr>
                            <div class="invoice-body">
                                <h5>Detail {{ ucReplaceUnderscoreToSpace('mutasi_antar_gudang') }}:</h5>
                                <br>
                                <table class="table table-bordered">
                                    ...
                                    <tbody>
                                        @foreach ($mutasi_antar_gudang_detail as $index => $material)
                                            <tr>
                                                <td>{{ $index + 1 }}</td>
        ...
        function exportToPDF() {
            ...
            html2pdf()
                ...
                .set({
                    ...
                    filename: "Faktur {{ ucReplaceUnderscoreToSpace('mutasi_antar_gudang') }}.pdf",
