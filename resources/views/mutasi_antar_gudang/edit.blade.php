...
@section('title')
    Edit {{ ucReplaceUnderscoreToSpace('mutasi_antar_gudang') }}
@endsection
...
@section('navigation')
    <ul class="breadcrumbs">
        ...
        <li class="nav-item">
            <a href="{{ route('mutasi_antar_gudang.index') }}">{{ ucReplaceUnderscoreToSpace('mutasi_antar_gudang') }}</a>
        </li>
        <li class="separator">
            <i class="icon-arrow-right"></i>
        </li>
        <li class="nav-item">
            <a href="{{ route('mutasi_antar_gudang.index') }}">@yield('title')</a>
        </li>
    </ul>
@endsection
...
@section('content')
        ...
        <div class="page-inner">
            ...
            <div class="row">
                <div class="col-md-12">
                    <div class="card">
                        <div class="card-body">
                            <form action="{{ route('mutasi_antar_gudang.update', $data->id) }}" method="POST">
                                @csrf
                                @method('PUT')
                                <div class="row">
                                    <div class="col-md-4">
                                        ...
                                        {{-- <div class="form-group">
                                            <label for="jenis_mutasi_antar_gudang_id">{{ ucReplaceUnderscoreToSpace('jenis_mutasi_antar_gudang') }}</label>
                                            <select class="jenis_mutasi_antar_gudang form-control @error('jenis_mutasi_antar_gudang_id') is-invalid @enderror" id="jenis_mutasi_antar_gudang_id" name="jenis_mutasi_antar_gudang_id" required>
                                                <option disabled selected>Pilih {{ ucReplaceUnderscoreToSpace('jenis_mutasi_antar_gudang') }}</option>
                                                @foreach ($jenis_mutasi_antar_gudangs as $jenis_mutasi_antar_gudang)
                                                    <option value="{{ $jenis_mutasi_antar_gudang->id }}" {{ $data->jenis_mutasi_antar_gudang_id == $jenis_mutasi_antar_gudang->id ? 'selected' : '' }} data-kode-jurnal-gudang="{{ $jenis_mutasi_antar_gudang->kode }}">
                                                        {{ ucwords(str_replace('_', ' ', $jenis_mutasi_antar_gudang->nama)) }}
                                                    </option>
                                                @endforeach
                                            </select>
                                            @error('jenis_mutasi_antar_gudang_id')
                                    ...
                                    <div class="col-md-4">
                                        <div class="form-group">
                                            <label for="gudang_asal_id">{{ ucReplaceUnderscoreToSpace('gudang_asal') }}</label>
                                            <select class="gudang form-control @error('gudang_asal_id') is-invalid @enderror" id="gudang_asal_id" name="gudang_asal_id" required>
                                                <option disabled selected>Pilih {{ ucReplaceUnderscoreToSpace('gudang') }}</option>
                                                @foreach ($gudangs as $gudang)
                                                    <option value="{{ $gudang->id }}" {{ $data->gudang_asal_id == $gudang->id ? 'selected' : '' }}>
                                    ...
                                    <div class="col-md-4">
                                        <div class="form-group">
                                            <label for="gudang_tujuan_id">{{ ucReplaceUnderscoreToSpace('gudang_tujuan') }}</label>
                                            <select class="gudang form-control @error('gudang_tujuan_id') is-invalid @enderror" id="gudang_tujuan_id" name="gudang_tujuan_id" required>
                                                <option disabled selected>Pilih {{ ucReplaceUnderscoreToSpace('gudang') }}</option>
                                                @foreach ($gudangs as $gudang)
                                                    <option value="{{ $gudang->id }}" {{ $data->gudang_tujuan_id == $gudang->id ? 'selected' : '' }}>
                                                        {{-- {{ ucwords(str_replace('_', ' ', $gudang->kode)) }} | --}}
                                                        {{ ucwords(str_replace('_', ' ', $gudang->nama)) }}
                                                    </option>
                                                @endforeach
                                        ...
                                        <div class="table-responsive">
                                            <table class="table table-bordered">
                                                ...
                                                <tbody id="materials_container">
                                                    @foreach ($mutasi_antar_gudang_detail as $index => $mutasi_antar_gudang)
                                                        <tr id="material_row_{{ $index }}">
                                                            <td>
                                                                <select class="material form-control" id="material_{{ $index }}" name="materials[]" required>
                                                                    ...
                                                                    @foreach ($materials as $item)
                                                                        <option value="{{ $item->id }}" {{ $mutasi_antar_gudang->material_id == $item->id ? 'selected' : '' }}
                                                            ...
                                                            <td>
                                                                <input type="number" class="form-control jumlah_dalam_satuan_kecil" id="jumlah_dalam_satuan_kecil_{{ $index }}" name="jumlah_kecil[]" placeholder="Jumlah Kecil" value="{{ $mutasi_antar_gudang->jumlah_dalam_satuan_kecil }}" step="any" required>
                                                                <span id="satuan_kecil_{{ $index }}" class="satuan_kecil">{{ $mutasi_antar_gudang->material->satuan_kecil->nama }}</span>
                                                            </td>
                                                            <td>
                                                                <input type="number" class="form-control jumlah_dalam_satuan_besar" id="jumlah_dalam_satuan_besar_{{ $index }}" name="jumlah_besar[]" placeholder="Jumlah Besar" value="{{ $mutasi_antar_gudang->jumlah_dalam_satuan_besar }}" step="any">
                                                                <span id="satuan_besar_{{ $index }}" class="satuan_besar">{{ $mutasi_antar_gudang->material->satuan_besar->nama }}</span>
                                                            </td>
                                                            <td>
    ...
    <script>
        $(document).ready(function() {
            ...
            let materialCounter = {{ $mutasi_antar_gudang_detail->count() }};
