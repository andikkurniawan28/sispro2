...
@section('title')
    {{ ucReplaceUnderscoreToSpace('mutasi_antar_gudang') }}
@endsection
...
@section('content')
                    ...
                    <div class="card">
                        <div class="card-header">
                            <div class="card-title text-white text-right">
                                <a href="{{ route('mutasi_antar_gudang.create') }}" class="btn btn-sm btn-primary text-white">Tambah</a>
                            </div>
                        </div>
                        <div class="card-body">
    ...
    <script type="text/javascript">
        $(function () {
            $('#mutasi_antar_gudang-table').DataTable({
                order: [
                    [1, 'desc']
                ],
                processing: true,
                serverSide: true,
                ajax: "{{ route('mutasi_antar_gudang.index') }}",
                columns: [
                    { data: 'kode', name: 'kode' },
                    { data: 'created_at', name: 'created_at' },
                    { data: 'gudang_asal_nama', name: 'gudang_asal_nama' },
                    { data: 'gudang_tujuan_nama', name: 'gudang_tujuan_nama' },
        ...
        document.addEventListener("DOMContentLoaded", function() {
            document.addEventListener('click', function(event) {
                if (event.target.classList.contains('delete-btn')) {
                    ...
                    const mutasi_antar_gudang_id = button.getAttribute('data-id');
                    const mutasi_antar_gudang_nama = button.getAttribute('data-nama');
                    ...
                    Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            ...
                            form.setAttribute('action', `{{ url('mutasi_antar_gudang') }}/${mutasi_antar_gudang_id}`);
