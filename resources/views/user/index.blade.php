...
@section('title')
    {{ ucReplaceUnderscoreToSpace('user') }}
@endsection
                    ...
                    <div class="card">
                        <div class="card-header">
                            <div class="card-title text-white text-right">
                                <a href="{{ route('user.create') }}" class="btn btn-sm btn-primary text-white">Tambah</a>
                            </div>
                        </div>
                        <div class="card-body">
                            <div class="table-responsive">
                                <table class="table table-bordered table-hover mt-4" id="user-table" width="100%">
                                    <thead>
                                        <tr>
                                            <th>{{ ucReplaceUnderscoreToSpace('nama') }}</th>
                                            <th>{{ ucReplaceUnderscoreToSpace('jabatan') }}</th>
                                            <th>{{ ucReplaceUnderscoreToSpace('username') }}</th>
                ...
                if (event.target.classList.contains('delete-btn')) {
                    ...
                    Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            ...
                            form.setAttribute('action', `{{ url('user') }}/${user_id}`);
