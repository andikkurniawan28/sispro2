...
@section('title')
    Edit {{ ucReplaceUnderscoreToSpace('pengeluaran_gudang') }}
@endsection
...
@section('content')
        ...
        <div class="page-inner">
            ...
            <div class="row">
                <div class="col-md-12">
                    <div class="card">
                        <div class="card-body">
                            <form action="{{ route('pengeluaran_gudang.update', $data->id) }}" method="POST">
                                @csrf
                                @method('PUT')
                                <div class="row">
                                    <div class="col-md-4">
                                        ...
                                        {{-- <div class="form-group">
                                            <label for="jenis_pengeluaran_gudang_id">{{ ucReplaceUnderscoreToSpace('jenis_pengeluaran_gudang') }}</label>
                                            <select class="jenis_pengeluaran_gudang form-control @error('jenis_pengeluaran_gudang_id') is-invalid @enderror" id="jenis_pengeluaran_gudang_id" name="jenis_pengeluaran_gudang_id" required>
                                                <option disabled selected>Pilih {{ ucReplaceUnderscoreToSpace('jenis_pengeluaran_gudang') }}</option>
                                                @foreach ($jenis_pengeluaran_gudangs as $jenis_pengeluaran_gudang)
                                                    <option value="{{ $jenis_pengeluaran_gudang->id }}" {{ $data->jenis_pengeluaran_gudang_id == $jenis_pengeluaran_gudang->id ? 'selected' : '' }} data-kode-jurnal-gudang="{{ $jenis_pengeluaran_gudang->kode }}">
                                                        {{ ucwords(str_replace('_', ' ', $jenis_pengeluaran_gudang->nama)) }}
                                                    </option>
                                                @endforeach
                                            </select>
                                            @error('jenis_pengeluaran_gudang_id')
                                        ...
                                        <div class="table-responsive">
                                            <table class="table table-bordered">
                                                ...
                                                <tbody id="materials_container">
                                                    @foreach ($pengeluaran_gudang_detail as $index => $pengeluaran_gudang)
                                                        <tr id="material_row_{{ $index }}">
                                                            <td>
                                                                <select class="material form-control" id="material_{{ $index }}" name="materials[]" required>
                                                                    ...
                                                                    @foreach ($materials as $item)
                                                                        <option value="{{ $item->id }}" {{ $pengeluaran_gudang->material_id == $item->id ? 'selected' : '' }}
                                                            ...
                                                            <td>
                                                                <input type="number" class="form-control jumlah_dalam_satuan_kecil" id="jumlah_dalam_satuan_kecil_{{ $index }}" name="jumlah_kecil[]" placeholder="Jumlah Kecil" value="{{ $pengeluaran_gudang->jumlah_dalam_satuan_kecil }}" step="any" required>
                                                                <span id="satuan_kecil_{{ $index }}" class="satuan_kecil">{{ $pengeluaran_gudang->material->satuan_kecil->nama }}</span>
                                                            </td>
                                                            <td>
                                                                <input type="number" class="form-control jumlah_dalam_satuan_besar" id="jumlah_dalam_satuan_besar_{{ $index }}" name="jumlah_besar[]" placeholder="Jumlah Besar" value="{{ $pengeluaran_gudang->jumlah_dalam_satuan_besar }}" step="any">
                                                                <span id="satuan_besar_{{ $index }}" class="satuan_besar">{{ $pengeluaran_gudang->material->satuan_besar->nama }}</span>
                                                            </td>
                                                            <td>
    ...
    <script>
        $(document).ready(function() {
            ...
            let materialCounter = {{ $pengeluaran_gudang_detail->count() }};
