...
@section('title')
    {{ ucReplaceUnderscoreToSpace('perintah_produksi') }}
@endsection
...
@section('content')
                    ...
                    <div class="card">
                        <div class="card-header">
                            <div class="card-title text-white text-right">
                                <a href="{{ route('perintah_produksi.create') }}" class="btn btn-sm btn-primary text-white">Tambah</a>
                            </div>
                        </div>
                        <div class="card-body">
                            <div class="table-responsive">
                                <table class="table table-bordered mt-4" id="perintah_produksi-table" width="100%">
                                    <thead>
                                        <tr>
    ...
    <script type="text/javascript">
        $(function () {
            $('#perintah_produksi-table').DataTable({
                order: [
                    [0, 'desc']
                ],
                processing: true,
                serverSide: true,
                ajax: "{{ route('perintah_produksi.index') }}",
                columns: [
                    { data: 'kode', name: 'kode' },
        ...
        document.addEventListener("DOMContentLoaded", function() {
            document.addEventListener('click', function(event) {
                if (event.target.classList.contains('delete-btn')) {
                    ...
                    const perintah_produksi_id = button.getAttribute('data-id');
                    const perintah_produksi_nama = button.getAttribute('data-nama');
                    ...
                    Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            ...
                            form.setAttribute('action', `{{ url('perintah_produksi') }}/${perintah_produksi_id}`);
