...
@section('title')
    Edit {{ ucReplaceUnderscoreToSpace('perintah_produksi') }}
@endsection
        ...
        <div class="page-inner">
            ...
            <div class="row">
                <div class="col-md-12">
                    <div class="card">
                        <div class="card-body">
                            <form action="{{ route('perintah_produksi.update', $data->id) }}" method="POST">
                                @csrf
                                @method('PUT')
                                <div class="row">
                                    <div class="col-md-4">
                                        ...
                                        <div class="table-responsive">
                                            <table class="table table-bordered">
                                                ...
                                                <tbody id="materials_container">
                                                    @foreach ($hasil_produksi as $index => $perintah_produksi)
                                                        <tr id="material_row_{{ $index }}">
                                                            <td>
                                                                <select class="material form-control" id="material_{{ $index }}" name="materials[]" required>
                                                                    ...
                                                                    @foreach ($materials as $item)
                                                                        <option value="{{ $item->id }}" {{ $perintah_produksi->material_id == $item->id ? 'selected' : '' }}
                                                            ...
                                                            <td>
                                                                <input type="number" class="form-control jumlah_dalam_satuan_kecil" id="jumlah_dalam_satuan_kecil_{{ $index }}" name="jumlah_kecil[]" placeholder="Jumlah Kecil" value="{{ $perintah_produksi->jumlah_dalam_satuan_kecil }}" step="any" required>
                                                                <span id="satuan_kecil_{{ $index }}" class="satuan_kecil">{{ $perintah_produksi->material->satuan_kecil->nama }}</span>
                                                            </td>
                                                            <td>
                                                                <input type="number" class="form-control jumlah_dalam_satuan_besar" id="jumlah_dalam_satuan_besar_{{ $index }}" name="jumlah_besar[]" placeholder="Jumlah Besar" value="{{ $perintah_produksi->jumlah_dalam_satuan_besar }}" step="any">
                                                                <span id="satuan_besar_{{ $index }}" class="satuan_besar">{{ $perintah_produksi->material->satuan_besar->nama }}</span>
                                                            </td>
                                                            <td>
