...
@section('title')
    Detail {{ ucReplaceUnderscoreToSpace('kebutuhan_produk_reproses_untuk_produk_akhir') }}
@endsection
                    ...
                    <div class="card" id="invoice-section">
                        <div class="card-body">
                            <div class="invoice-header">
                                <h4 class="invoice-title">{{ ucReplaceUnderscoreToSpace('kebutuhan_produk_reproses_untuk_produk_akhir') }}</h4>
                                ...
                                <table class="table table-bordered">
                                    ...
                                    <tbody>
                                        @foreach ($kebutuhan_produk_reproses_untuk_produk_akhir as $index => $kebutuhan)
                                            <tr>
                                                <td>{{ $index + 1 }}</td>
                                                <td>{{ ucReplaceUnderscoreToSpace($kebutuhan->produk_reproses->kode) }}</td>
                                                <td>{{ ucReplaceUnderscoreToSpace($kebutuhan->produk_reproses->nama) }}</td>
                                                <td>{{ $kebutuhan->jumlah_dalam_satuan_kecil }}</td>
                                                <td>{{ $kebutuhan->produk_reproses->satuan_kecil->nama }}</td>
                                                <td>{{ $kebutuhan->jumlah_dalam_satuan_besar }}</td>
                                                <td>{{ $kebutuhan->produk_reproses->satuan_besar->nama }}</td>
                                            </tr>
                                        @endforeach
                                    </tbody>
                                </table>
        ...
        function exportToPDF() {
            ...
            html2pdf()
                ...
                .set({
                    ...
                    filename: "Faktur {{ ucReplaceUnderscoreToSpace('kebutuhan_produk_reproses_untuk_produk_akhir') }}.pdf",
